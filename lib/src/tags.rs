...
impl<'a> TagsRef<'a> {
    ...
    fn choose<T, TId3v1, TId3v2, TVComment>(&self,
        id3v1: TId3v1,
        id3v2: TId3v2,
        vcomment: TVComment,
    ) -> Option<T>
        where TId3v1: FnOnce(&'a Id3v1) -> Option<T>,
              ...
              TVComment: FnOnce(&'a Vcomment) -> Option<T>,
    {
        if let Some(v) = self.vcomment {
            vcomment(v)
        } else if let Some(v) = self.id3v2 {
            id3v2(v)
